...
```Java
    ...
    void 失敗例_toInstantで比較すると意図しない結果になる() {
        ...
        // この比較は論理的に誤り！
        // LocalDateTime.now()はSJVMのデフォルトタイムゾーンの時刻を返すのに、それをJSTとして解釈してしまっている
...
1. 社内でのプロダクト連携を進めており、仮に日時情報を連携するとなった場合、その日時のタイムゾーンを保証しておく必要がある。
...
* JVM上で動作するアプリケーションにできるだけタイムゾーンの制御を集約したい。
